...
class MediaCorpus {
public:
	DROIDCLI_API bool load_pair(
		const core::String& pdf_text_md_path,
		const core::String& objs_text_md_path);

	DROIDCLI_API bool load_from_directory(const core::String& data_directory);

	DROIDCLI_API bool empty() const { return entries_.empty(); }
	DROIDCLI_API std::size_t size() const { return entries_.size(); }

	DROIDCLI_API const ImageCorpusEntry* find_by_file_key(const core::String& file_key) const;
	DROIDCLI_API const ImageCorpusEntry* find_by_path(const core::String& absolute_or_display_path) const;

	/// Full OCR text for subtitles / downstream LLM streaming.
	DROIDCLI_API core::String subtitle_text_for(const core::String& file_key) const;

	/// Short line for on-screen overlay.
	DROIDCLI_API core::String subtitle_preview_for(
	...
	DROIDCLI_API bool focus_rect_pixels(
	...
	DROIDCLI_API std::size_t pick_focus_region_index(
		const ImageCorpusEntry& entry,
		std::size_t seed = 0) const;

	/// 16:9 crop centered on a single text region (pixel coords).
	DROIDCLI_API bool region_focus_crop_16x9(
		const ImageCorpusEntry& entry,
	...
	DROIDCLI_API bool build_region_mask(
		const ImageCorpusEntry& entry,
		int32_t mask_width,
		int32_t mask_height,
...
DROIDCLI_API core::String corpus_file_key_from_path(const core::String& path);
